...
class MultiAxisTrajectoryGenerator:
    ...
    Supports linear, cubic, quintic polynomial, trapezoidal, and spline velocity profiles.
    """
...
class Spline:
    """
    Cubic spline interpolation with position and velocity boundary constraints.
    Uses SciPy's CubicSpline for natural cubic spline generation.
    """
    ...
    def generate(self, nsteps=100):
        """
        Generate the spline trajectory for each DOF.
        ...
        for i in range(self.ndof):
            # Time points for control points (start and end)
            t_ctrl = [0, self.T]
            # Positions at control points
            pos_ctrl = [self.start_pos[i], self.final_pos[i]]

            # Create cubic spline with velocity boundary conditions
            cs = CubicSpline(
                t_ctrl,
                pos_ctrl,
                bc_type=((1, self.start_vel[i]), (1, self.final_vel[i])),
            )

            # Evaluate position, velocity, acceleration
            q = cs(self.t)
            qd = cs(self.t, 1)  # First derivative
            qdd = cs(self.t, 2)  # Second derivative

            self.X[i] = [q.tolist(), qd.tolist(), qdd.tolist()]
